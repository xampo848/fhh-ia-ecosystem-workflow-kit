...
| `codebase-memory-mcp` | runtime capability already available | `.agents/capabilities/manifests/codebase-memory-mcp.md` | optional / attachable | Prefer for multi-hop discovery and graph impact checks when `mcp_codebase-memo_*` is exposed and index is ready; otherwise use native search directly | repository-native search and targeted reads |
