...
The generated matcher-first pattern catalog lives at `.agents/skills/06-patterns/index.md`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `workflow-router` | Workflow | `.agents/skills/00-router/workflow-router/SKILL.md` | Per-turn intake selects non-trivial, iterative, implementation-adjacent, or multi-step freeform work without an explicit skill, and provides explicit route options when user choice is required | Startup-minimal | lean | `workflow-router` | Intake runs every prompt; load the full skill for non-trivial, iterative, implementation-adjacent, or multi-step routing, including user-choice menus (for example `project-formation` vs `create-prd`). |
...
| `project-formation` | Workflow | `.agents/skills/01-product/project-formation/SKILL.md` | PM needs end-to-end project shaping from discovery through GTM and dossier handoff | Explicit-only | balanced | `project-formation` | Clean-room product workflow for project formation without external copied content. |
| `solution-sketch` | Workflow | `.agents/skills/01-product/solution-sketch/SKILL.md` | User asks to compare solution shapes, classes, or architecture before writing a PRD or epic | Explicit-only | lean | `solution-sketch` | Offer-only sister skill; never auto-start; writes `docs/design/<slug>-sketch.md`. |
| `project-formation-router` | Delegate-only implementation | `.agents/skills/01-product/project-formation/router/SKILL.md` | `project-formation` needs per-turn internal stage routing and anti-skip gating | Delegated-only | lean | `project-formation-router` | Owned by `project-formation`. |
| `project-formation-discovery` | Delegate-only implementation | `.agents/skills/01-product/project-formation/discovery/SKILL.md` | `project-formation` needs user/stakeholder discovery and question design | Delegated-only | lean | `project-formation-discovery` | Owned by `project-formation`. |
| `project-formation-shaping` | Delegate-only implementation | `.agents/skills/01-product/project-formation/shaping/SKILL.md` | `project-formation` needs appetite-based shaping and tradeoff decisions | Delegated-only | balanced | `project-formation-shaping` | Owned by `project-formation`. |
| `project-formation-roadmap` | Delegate-only implementation | `.agents/skills/01-product/project-formation/roadmap/SKILL.md` | `project-formation` needs phased sequencing, dependencies, and risk gating | Delegated-only | balanced | `project-formation-roadmap` | Owned by `project-formation`. |
| `project-formation-gtm` | Delegate-only implementation | `.agents/skills/01-product/project-formation/gtm/SKILL.md` | `project-formation` needs go-to-market planning and KPI framing | Delegated-only | balanced | `project-formation-gtm` | Owned by `project-formation`. |
| `project-formation-dossier` | Delegate-only implementation | `.agents/skills/01-product/project-formation/dossier/SKILL.md` | `project-formation` needs complete project dossier assembly for execution handoff | Delegated-only | lean | `project-formation-dossier` | Owned by `project-formation`. |
...
| `playwright-testing` | Quality/validation | `.agents/skills/03-quality/playwright-testing/SKILL.md` | User asks for Playwright tests, or most navigable user-facing UI changes require E2E by default when tooling is available | Just-in-time | balanced | `playwright-testing` | Stack-gated: default-required for most navigable user-facing UI flows when tooling is available; explicit documented exceptions or user waivers required to skip. |
